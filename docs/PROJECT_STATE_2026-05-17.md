...
- `Config.gs` podnosi zapis do wersji `5`; stan ma `history.dailyGrowth` oraz rozszerzone `attention.pausedUntil` i `attention.quietSuppressed`.
...
- `ClientCoreCare.html` segmentuje upływ czasu, pauzuje attention w nocy, przesuwa deadline na poranną karencję i formatuje dłuższe cooldowny.
