...
- Zmniejszono i oczyszczono zarodnik po imagegen, bez kwadratowej doniczki i bez magentowych obwodek.
...
- `audit-sprite-consistency`: OK, `sleep`, `wake` i `idle` trzymaja rozmiar w kazdym etapie.
- test magenty: `exact_ff00ff=0`, `magenta_like_visible=0` dla `108` runtime PNG.
...
- asercje animacji: wszystkie testowane akcje renderuja klucze `stage.activity.action`, np. `spore.activity.hydrate`, `baby.activity.feed`, `young.activity.play`, bez przeskoku na `adult`.

## Pozostawione Swiadomie

- Globalne tlo strony nadal ma bardzo delikatna kratke dekoracyjna. Usuniety zostal checkerboard z obszaru sceny/canvasu, bo to wygladalo jak blad przezroczystosci.
- Efekty czasteczkowe nadal maja miekkie ostrzezenia driftu w walidatorze. To jest oczekiwany ruch efektow, nie drift postaci.
